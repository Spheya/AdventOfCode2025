...
int main() {
	...
	for (int i = 0; i < 1000; ++i) {
		...
		if (b->circuitId == 0) {
			...
			unsigned swapId = a->circuitId;
			for(auto& p : points)
				if(p.circuitId == swapId)
					p.circuitId = b->circuitId;
		}
	}

	std::vector<unsigned> circuitSizes(nextCircuitId);
	for(auto& p : points)
		if(p.circuitId != 0)
			++circuitSizes[p.circuitId];

	std::sort(circuitSizes.rbegin(), circuitSizes.rend());
	std::cout << circuitSizes[0] * circuitSizes[1] * circuitSizes[2] << std::endl;
}
